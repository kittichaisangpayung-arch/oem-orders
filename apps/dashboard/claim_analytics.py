...
@login_required
def claim_summary_dashboard(request):
    """Dashboard showing claim analytics and trends"""
    ...
    # Base queryset
    qs = Claim.objects.select_related('product', 'store', 'store__customer', 'purchase_order')
    ...
    # Overall stats
    total_claims = qs.aggregate(
        total_qty=Sum('qty'),
        total_count=Count('id')
    )

    # Top 10 most claimed products
    ...
    recent_claims = qs.order_by('-created_at')[:20]
    ...
    return render(request, 'dashboard/claim_summary_dashboard.html', context)
